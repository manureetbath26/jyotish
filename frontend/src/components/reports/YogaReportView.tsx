...
// ─── PDF Generation ──────────────────────────────────────────────────────────
const CATEGORY_LABEL_PDF: Record<YogaCategory, string> = {
  mahapurusha: "Pancha Mahapurusha Yogas",
...
const CATEGORY_DISPLAY_ORDER_PDF: YogaCategory[] = [
  "mahapurusha", "raja", "dhana", "nabhasa", "chandra", "surya", "special", "dosha",
];

async function downloadYogaReportPdf(detected: DetectedYoga[], chart: ChartResponse, name?: string) {
  ...
  const colors = {
    ...
    fuchsia: [192, 38,  211] as [number, number, number],
  ...
  const CATEGORY_COLOR: Record<YogaCategory, [number, number, number]> = {
    mahapurusha: colors.amber,
    nabhasa:     colors.cyan,
    chandra:     colors.indigo,
    surya:       colors.orange,
    raja:        colors.fuchsia,
    dhana:       colors.emerald,
    special:     colors.blue,
    dosha:       colors.rose,
  };
  ...
  function bodyText(text: string, maxWidth?: number, indent = 0) {
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...colors.text);
    const lines = doc.splitTextToSize(text, maxWidth ?? (contentW - indent));
    for (const line of lines) { checkPage(5); doc.text(line, margin + indent, y); y += 4; }
  }
  function categoryHeader(cat: YogaCategory, count: number) {
    checkPage(16);
    y += 4;
    const cc = CATEGORY_COLOR[cat];
    doc.setFillColor(cc[0] * 0.2 + 20, cc[1] * 0.2 + 30, cc[2] * 0.2 + 40);
    doc.roundedRect(margin, y - 5, contentW, 10, 2, 2, "F");
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...cc);
    doc.text(`${CATEGORY_LABEL_PDF[cat]}`, margin + 4, y + 1.5);
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(180, 180, 200);
    doc.text(`(${count})`, pageW - margin - 2, y + 1.5, { align: "right" });
    y += 10;
  }
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Yoga Report", pageW / 2, 16, { align: "center" });
  doc.setFontSize(9);
  ...
  doc.text(`Classical Vedic planetary combinations (BPHS chapters 34-43)  |  Generated: ${new Date().toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}`, pageW / 2, 35, { align: "center" });
  y = 50;
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...colors.amber);
  doc.text(`${detected.length} yogas detected`, margin, y);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor(...colors.emerald);
  doc.text(`${positiveCount} positive`, margin + 55, y);
  doc.setTextColor(...colors.rose);
  doc.text(`${doshaCount} doshas`, margin + 85, y);
  y += 8;
  ...
  // ═══ PER CATEGORY ═══
  const grouped = new Map<YogaCategory, DetectedYoga[]>();
  for (const cat of CATEGORY_DISPLAY_ORDER_PDF) grouped.set(cat, []);
  for (const d of detected) {
    const arr = grouped.get(d.rule.category) ?? [];
    arr.push(d);
    grouped.set(d.rule.category, arr);
  }

  for (const cat of CATEGORY_DISPLAY_ORDER_PDF) {
    const yogas = grouped.get(cat) ?? [];
    if (yogas.length === 0) continue;

    categoryHeader(cat, yogas.length);

    for (const d of yogas) {
      checkPage(20);
      // Yoga name + chapter + importance stars
      doc.setFontSize(10);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(...colors.text);
      doc.text(d.rule.name, margin, y);

      doc.setFontSize(8);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(...CATEGORY_COLOR[cat]);
      doc.text(`Ch. ${d.rule.chapter}`, pageW - margin, y, { align: "right" });
      y += 4.5;

      // Importance stars
      if (d.rule.importance) {
        doc.setFontSize(8);
        doc.setFont("helvetica", "bold");
        doc.setTextColor(...colors.amber);
        doc.text("★".repeat(d.rule.importance), margin, y);
        y += 4.5;
      }
      ...
      doc.setTextColor(...colors.gray);
      const evidenceLines = doc.splitTextToSize(d.evidence, contentW - 4);
      for (const line of evidenceLines) { checkPage(4); doc.text(line, margin + 4, y); y += 3.5; }
      y += 1;

      // Formation in this chart
      if (d.formationInChart) {
        bodyText(d.formationInChart, contentW - 4, 4);
        y += 1;
      }

      // House context note
      if (d.houseContextNote) {
        doc.setFontSize(7.5);
        doc.setFont("helvetica", "italic");
        doc.setTextColor(...colors.gray);
        const noteLines = doc.splitTextToSize(d.houseContextNote, contentW - 4);
        for (const line of noteLines) { checkPage(4); doc.text(line, margin + 4, y); y += 3.5; }
        y += 1;
      }

      // House verdict if available
      if (d.houseVerdict) {
        const verdictColor = d.houseVerdict === "favorable" ? colors.emerald
          : d.houseVerdict === "challenging" ? colors.rose
          : colors.cyan;
        doc.setFontSize(7.5);
        doc.setFont("helvetica", "bold");
        doc.setTextColor(...verdictColor);
        doc.text(d.houseVerdict.charAt(0).toUpperCase() + d.houseVerdict.slice(1), margin + 4, y);
        y += 4;
      }
  ...
  doc.setFont("helvetica", "italic");
  doc.setTextColor(...colors.gray);
  const note = "Source: Brihat Parashara Hora Shastra, chapters 34-43. Each yoga is detected from its classical formation rule. Doshas classically carry mitigations — read them in the context of the full chart.";
  const noteLines = doc.splitTextToSize(note, contentW);
  for (const line of noteLines) { checkPage(4); doc.text(line, margin, y); y += 3.5; }

  footer();
  const safeName = (name ?? "chart").replace(/[^a-z0-9]/gi, "_").toLowerCase();
  doc.save(`yoga_report_${safeName}.pdf`);
}
...
export function YogaReportView({ chart, userName, onBack, frozenReport, onReportReady }: Props) {
  ...
  const handleDownload = useCallback(() => {
    ...
    return downloadYogaReportPdf(detected, chart, userName);
  }, [detected, chart, userName]);
  ...
  return (
    <ReportShell
      ...
      onDownload={detected.length > 0 ? handleDownload : undefined}
    >
