...
import { YogaReportView } from "@/components/reports/YogaReportView";
import { ProfileSelector, type SelectedSource } from "@/components/ProfileSelector";
import { useActiveProfile } from "@/contexts/ActiveProfileContext";
import type { DetectedYoga } from "@/lib/yogaEngine";
...
function YogaReportContent() {
  ...
  const [frozenReport, setFrozenReport] = useState<DetectedYoga[] | null>(null);
  ...
  useEffect(() => {
    fetch("/api/reports/catalog")
      ...
      .then((entries: { slug: string; price: number }[]) => {
        const entry = entries.find((e) => e.slug === "yoga");
    ...
    fetch(`/api/reports/purchase/${reportId}`)
      ...
      .then((data) => {
        if (data?.chartData && data.status === "verified") {
          ...
          if (data.reportData) setFrozenReport(data.reportData as DetectedYoga[]);
          setStep("report");
        }
      })
      .catch(() => {})
  ...
  const handleReportReady = useCallback(
    (detected: DetectedYoga[]) => {
      if (!purchaseId) return;
      fetch(`/api/reports/purchase/${purchaseId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ reportData: detected }),
      }).catch(() => {});
    },
    [purchaseId],
  );
  ...
  const upiLink = buildUpiLink(reportPrice, "Jyotish Yoga Report");
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      <header className="text-center py-4">
        <h1 className="text-2xl font-bold text-amber-400 flex items-center justify-center gap-2">
          ...
          Yoga Report &mdash; Vedic Astrology
        </h1>
        <p className="text-slate-400 text-sm mt-1">
          Discover the classical planetary combinations present in your chart
        </p>
      </header>
      ...
      {step === "birth" && (
        <form onSubmit={handleCalculate} className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5">
          ...
          <p className="text-xs text-slate-500 leading-relaxed">
            Yogas are detected from exact planetary positions, so accurate birth time and place matter.
          </p>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            ...
            <div className="bg-slate-800/50 rounded-xl p-4 space-y-2">
              ...
              <p className="text-xs text-slate-400">
                Full Yoga report covers 40+ classical combinations from BPHS chapters 34-43.
                Each detected yoga includes the formation rule, what it confers, the classical
                source text, and supporting evidence from your chart (which planets, which houses).
              </p>
              <div className="grid grid-cols-2 gap-2 mt-3">
                {[
                  "Pancha Mahapurusha (Ruchaka, Bhadra, Hamsa, Malavya, Sasha)",
                  "Nabhasa Yogas (Asraya, Dala, Sankhya)",
                  "Gaja Kesari, Amala, Parvata, Lakshmi",
                  "Chandra Yogas (Adhi, Sunapha, Anapha, Duradhara)",
                  "Surya Yogas (Vesi, Vosi, Ubhayachari)",
                  "Raja Yogas (Maha Raja, Kendra-Kona)",
                  "Dhana (Wealth) Yogas",
                  "Doshas (Kema Druma, Kala Sarpa, Shakata, Daridra)",
                ].map((f) => (
      ...
      {step === "payment" && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5">
          ...
          <p className="text-sm text-slate-400">
            Pay {"\u20B9"}{reportPrice} via UPI to receive your complete Yoga Report.
          </p>
